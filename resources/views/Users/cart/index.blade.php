...
@push('styles')
  <style>
    ...
    .breadcrumb-chevron .breadcrumb-item+.breadcrumb-item::before {
      font-size: 22px;
      /* <— tăng/giảm tùy ý (18–26px) */
      font-weight: 500;
      line-height: 1;
      color: #6c757d;
      /* màu xám; đổi sang #f97316 nếu muốn cam */
      position: relative;
      top: -1px;
      /* chỉnh viền dọc cho cân */
      padding-right: .75rem;
      /* nới khoảng cách */
    }
  ...
  <div class="container">
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-8">
        <div class="cart-card">
          {{-- Desktop Header --}}
          <div class="cart-thead px-4 py-3 d-none d-lg-grid" style="grid-template-columns: 1fr 120px 120px 140px;">
            <div>Sản phẩm</div>
          ...
          <div class="p-4">
            ...
            @forelse ($items as $index => $it)
              @php
                ...
                $priceWithVAT = $price * 1.05;       // đơn giá sau VAT 5%
                $lineTotal = $priceWithVAT * $qty; // thành tiền sau VAT
              ...
              <div class="cart-item d-lg-grid align-items-center py-4"
                ...
                <div class="product-info d-flex align-items-start gap-3">
                  ...
                  <div class="flex-grow-1">
                    <a class="product-name d-block mb-2" href="{{ route('users.products.show', $p->slug) }}">
                      {{ $p->product_name }}
                    </a>
                    ...
                    <div class="d-flex align-items-center mb-3">
                      <div class="mini-thumb">
                        <img src="{{ product_hover_src($p) ?? product_main_src($p) }}" alt="Gift item" loading="lazy">
                      </div>
                      <div class="small">
                        <div class="fw-semibold">Tặng kèm: {{ $p->product_name }} Mini 30ml</div>
                        <div class="text-success">
                          Trị giá: <span
                            class="fw-semibold">{{ number_format(max(0, round($price * 0.6)), 0, ',', '.') }}₫</span>
                        </div>
                      </div>
                    </div>

                    {{-- Remove Button (đúng route + không dùng $id chưa khai báo) --}}
                    <form method="POST" action="{{ route('users.cart.remove', $rowId) }}"
...
<div class="price-current">{{ number_format($priceWithVAT,0,',','.') }}₫</div>
                ...
                <div class="text-lg-center mb-3 mb-lg-0">
                  <form method="POST" action="{{ route('users.cart.update', $rowId) }}">
                    @csrf @method('PATCH')
                    <input type="number" name="quantity" class="qty-input form-control" min="1" max="99" value="{{ $qty }}"
...
<div class="summary-row">
    <span>VAT (5%)</span>
    <span class="fw-bold">
        {{ number_format($subtotalWithVat - $subtotal, 0, ',', '.') }}₫
    </span>
</div>
...
<div class="summary-row summary-total">
    ...
    <span class="total-amount">
        {{ number_format($subtotalWithVat, 0, ',', '.') }}₫
    </span>
</div>
