...
@push('styles')
  <style>
    ...
    .svc-actions .btn-call {
      background: #0b8a2a;
      border: none
    }

    .svc-actions .btn-book {
      background: #ff7a00;
      border: none
    }
    ...
    .popular-list {
      ...
      /* bỏ bullet */
      padding-left: 0;
      /* bỏ thụt đầu dòng mặc định */
      margin: 0;
    }

    .popular-item {
      list-style: none;
      /* phòng khi browser vẫn render */
    }
    ...
    aside.sticky-top {
      max-height: 100vh;
      /* Giới hạn chiều cao để tránh overflow khi sticky */
      overflow-y: auto;
      /* Scroll nội dung sidebar nếu quá dài */
    }
  </style>
@endpush
            ...
            <form class="bottom:20px;" method="GET" action="{{ route('users.services.index') }}">
              ...
              <div class="mb-3">
                ...
                <div class="d-flex gap-2">
                  <input type="number" class="form-control" name="min_price" placeholder="Từ" value="{{ $min ?: '' }}"
                    step="10000" min="0">
                  <input type="number" class="form-control" name="max_price" placeholder="Đến" value="{{ $max ?: '' }}"
                    step="10000" min="0">
                </div>
              </div>
          ...
          <a href="{{ route('users.services.show', $sv->slug) }}" class="position-relative d-block svc-thumb-container">
              <img class="svc-thumb"
                ...
                <div class="svc-body d-flex flex-column">
                  ...
                  <h6 class="mt-1 mb-2">
                    <a class=" svc-title text-decoration-none text-dark"
                      href="{{ route('users.services.show', $sv->slug) }}">{{ $sv->service_name }}</a>
                  </h6>
                  ...
                  <p class="svc-desc mb-3">
                    {{ \Illuminate\Support\Str::limit($sv->short_desc ?? strip_tags($sv->description), 110) }}</p>

                  <div class="d-flex gap-2 svc-actions mt-auto">
                    <a href="tel:18006778" class="text-white btn btn-sm btn-call flex-grow-1"> <i
                        class="bi bi-telephone-fill" aria-hidden="true"></i>
                      18006778<br> <small>(nhấn phím 2)</small>
                    </a>
                    <a href="{{ route('users.booking.create', ['service' => $sv->slug]) }}"
                      class="btn btn-sm btn-book text-white text">
                      Đặt hẹn
                    </a>
                  </div>
                </div>
