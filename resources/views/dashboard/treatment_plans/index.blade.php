...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 w-full">
    ...
    <form action="{{ route('admin.treatment-plans.index') }}" method="GET"
        ...
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Tìm kiếm theo khách hàng, dịch vụ..."
    ...
    <form method="GET" action="{{ route('admin.treatment-plans.index') }}"
        class="d-flex gap-2 align-items-end bg-white rounded-xl shadow-lg px-3 py-2 border border-blue-100 ms-auto">
        ...
        <a href="{{ route('admin.treatment-plans.index') }}"
            class="px-3 py-2 bg-gray-50 text-gray-700 rounded-lg">Xoá lọc</a>
    </form>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($plan->packageService)
                                    <span class="badge bg-primary-subtle text-primary border border-primary-subtle rounded-pill">Gói
                                        Dịch vụ</span>
                                    <div class="text-sm mt-1">{{ $plan->packageService->service_name }}</div>
                                @elseif($plan->singleService)
                                    <span
                                        class="badge bg-secondary-subtle text-secondary border border-secondary-subtle rounded-pill">Dịch
                                        vụ Lẻ</span>
                            ...
                            <td>
                                <span
                                    class="badge bg-light text-dark border border-secondary-subtle">{{ $plan->sessions_count ?? $plan->sessions()->count() }}
                                    buổi</span>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <a href="{{ route('admin.treatment-plans.show', $plan->id) }}" class="btn btn-info"
                                        title="Xem chi tiết">
                ...
                <div class="bg-white border-t border-gray-200 px-6 py-4">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            {{-- Nút Trước --}}
                            <a href="{{ $plans->previousPageUrl() ?? '#' }}" class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-700 
                       {{ $plans->onFirstPage() ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}">
                            ...
                            {{-- Nút Sau --}}
                            <a href="{{ $plans->nextPageUrl() ?? '#' }}" class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-700 
