...
<div class="container-fluid">
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                <h1 class="mb-2" style="font-size:2.5rem;font-weight:700;">
                    <i class="fas fa-calendar-check mr-3"></i>Chi tiết kế hoạch #{{ $plan->id }}
                </h1>
            </div>
            <div class="col-md-4">
                <div class="d-flex justify-content-md-end gap-2">
                    <a href="{{ route('admin.treatment-plans.index') }}" class="btn-excel">
    ...
    <div class="card mb-4 border-0 shadow-sm">
        <div class="card-body p-4">
            <div class="row">
                <div class="col-md-6">
                    <p class="mb-2"><strong>Khách hàng:</strong> <span class="text-dark">{{ $plan->customer->name ?? ('KH#'.$plan->customer_id) }}</span></p>
                    <p class="mb-2">
                        <strong>Dịch vụ/Gói:</strong>
                        @if($plan->packageService)
                            <span class="badge bg-info-subtle text-info border border-info-subtle rounded-pill me-1">Gói</span>
                            <span class="text-dark">{{ $plan->packageService->service_name }}</span>
                        @elseif($plan->singleService)
                            <span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle rounded-pill me-1">Lẻ</span>
                            <span class="text-dark">{{ $plan->singleService->service_name }}</span>
                        @else
                            <span class="text-muted">-</span>
                        @endif
                    </p>
                    <p class="mb-2"><strong>Ngày bắt đầu:</strong> <span class="text-dark">{{ Carbon::parse($plan->start_date)->format('d/m/Y') }}</span></p>
                    <p class="mb-0"><strong>Trạng thái:</strong> @include('dashboard.treatment_plans._status-badge', ['status' => $plan->status])</p>
                </div>
                <div class="col-md-6">
                    @if($plan->note)
                        <p class="mb-0"><strong>Ghi chú:</strong> <span class="text-muted">{{ $plan->note }}</span></p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="sessions-table-wrapper">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0 fw-semibold">
                <i class="fas fa-list-ul me-2 text-primary"></i>Các buổi điều trị
            </h5>
        </div>
                        ...
                        <tr class="session-row">
                            ...
                            <td>
                                <div class="time-info">
                                    <div class="date-time">
                                        ...
                                        <span class="fw-semibold">{{ Carbon::parse($s->scheduled_start)->format('d/m/Y') }}</span>
                                    </div>
                                    <div class="time-range">
                                        <i class="fas fa-clock me-2 text-muted"></i>
                                        <span>{{ Carbon::parse($s->scheduled_start)->format('H:i') }}</span>
                                        <i class="fas fa-arrow-right mx-1 small text-muted"></i>
                                        <span>{{ Carbon::parse($s->scheduled_end)->format('H:i') }}</span>
                                        <span class="duration-badge ms-2">
                                            {{ Carbon::parse($s->scheduled_start)->diffInMinutes(Carbon::parse($s->scheduled_end)) }} phút
                                        </span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="action-buttons-small">
                                    {{-- Nút chỉnh sửa buổi --}}
                                    <a href="{{ route('admin.tsessions.edit', $s->id) }}" 
...
/* Reuse styles from sample for consistency */
.btn-excel {
