                        ...
                        @forelse($categories as $category)
                            ...
                            <tr class="hover:bg-blue-50 transition-colors duration-150">
                               ...
                               <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $category->status ? 'bg-emerald-200 text-emerald-900' : 'bg-red-200 text-red-900' }} badge-glow hover:shadow-md hover:{{ $category->status ? 'bg-emerald-300' : 'bg-red-300' }} transition-all duration-200">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form action="{{ route('admin.service_categories.destroy', $category->category_id) }}" method="POST" style="display:inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa danh mục này?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="inline-flex items-center px-3 py-2 bg-red-100 text-red-800 rounded-lg hover:bg-red-200 hover:scale-105 transition-all duration-200 text-sm font-medium">
                            ...
                            @if($category->children && $category->children->count() > 0)
                                @foreach($category->children as $child)
                                    <tr class="bg-gray-50 hover:bg-blue-50 transition-colors duration-150 group">
                                        <td class="px-6 py-4">
                                            <img src="{{ src_img_get(url: $child->image) }}" {{-- Example: Child category image --}}
                                                 alt="{{ $child->category_name }}"
                                                 class="w-12 h-12 object-cover rounded-lg ring-1 ring-gray-200 p-1.5 bg-white"
                                               >
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $child->status ? 'bg-emerald-200 text-emerald-900' : 'bg-red-200 text-red-900' }} badge-glow hover:shadow-md hover:{{ $child->status ? 'bg-emerald-300' : 'bg-red-300' }} transition-all duration-200">
                                                <span class="w-2 h-2 rounded-full mr-2 {{ $child->status ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                                                {{ $child->status ? 'Hoạt động' : 'Ngưng hoạt động' }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end space-x-2">
                                                ...
                                                <form action="{{ route('admin.service_categories.destroy', $child->category_id) }}" method="POST" style="display:inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa danh mục này?')">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="inline-flex items-center px-3 py-2 bg-red-100 text-red-800 rounded-lg hover:bg-red-200 hover:scale-105 transition-all duration-200 text-sm font-medium">
                                                        <i class="fas fa-trash mr-1"></i> Xóa
                                                    </button>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                    ...
                                    <p class="text-lg">Không có danh mục dịch vụ nào</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<style>
    ...
    .page-title { font-size: 2.5rem; font-weight: 800; margin-bottom: 0.5rem; position: relative; z-index: 2; }
    .page-subtitle { font-size: 1.1rem; opacity: 0.9; position: relative; z-index: 2; }
    .badge-glow { box-shadow: 0 0 8px rgba(59, 130, 246, 0.3); }
    .table-container { box-shadow: 0 4px 20px rgba(59, 130, 246, 0.15); }
...
function toggleDropdown(categoryId) {
    ...
    allDropdowns.forEach(dd => { if (dd.id !== `dropdown-${categoryId}`) dd.classList.add('hidden'); });
