...
@section('content')
  <style>
    ...
    .table-modern {
      ...
      width: 100%;
      table-layout: fixed;
      /* RẤT QUAN TRỌNG: chia đều các cột */
    }
  ...
  <div class="container-fluid">
    ...
    <form method="get"
      class="d-flex flex-wrap gap-3 align-items-center bg-white rounded-xl shadow-lg px-4 py-3 border border-blue-100 mb-4">
        ...
        <tbody>
          @forelse($orders as $o)
                  ...
                  <tr>
                    ...
                    <td><span style="align-content: center"
                        class="status-badge {{ $payClass }}">{{ $payMap[$o->payment_status] ?? ucfirst($o->payment_status) }}</span>
                    </td>

                    <td><span style="align-content: center"
                        class="status-badge {{ $statusClass }}">{{ $statusMap[$o->status] ?? ucfirst($o->status) }}</span></td>
                    ...
                    <td>
                      <div class="d-flex justify-content-center gap-2">
                        ...
                        <a href="javascript:void(0)" class="btn btn-sm btn-warning text-white fw-bold"
                          style="border-radius:12px; padding: 6px 14px; box-shadow: 0 2px 8px rgba(255,193,7,.3);" onclick="openEditModal(
               {{ $o->id }},
               '{{ $o->status }}',
               '{{ $o->payment_status }}',
               `{!! addslashes($o->shipping_address ?? '') !!}`,
               '{{ $o->order_code ?? $o->id }}'
             )">
    ...
    @if($orders instanceof \Illuminate\Pagination\LengthAwarePaginator)
      <div class="bg-white border-top px-4 py-3 d-flex justify-content-between align-items-center">
        <div class="text-sm text-gray-700">
          Hiển thị <span class="fw-semibold">{{ $orders->firstItem() }}</span>–<span
            class="fw-semibold">{{ $orders->lastItem() }}</span>
      ...
      <div class="modal-content rounded-4 shadow-xl border-0">
        ...
        <div class="modal-header border-0 text-white"
          ...
          <h5 class="modal-title fw-bold fs-5">
            <i class="fas fa-edit"></i> Chỉnh sửa đơn hàng <span id="modalOrderCode" class="text-warning">#000000</span>
          </h5>
        ...
        <form id="editOrderForm" method="POST">
          ...
          <div class="modal-body p-4">
            <div class="alert alert-primary text-center mb-4">
              <strong>Đơn hàng:</strong> <span id="displayOrderCode">#000000</span>
            </div>

            <div class="row g-4">
              <div class="col-md-6">
                <label class="fw-bold">Trạng thái đơn hàng</label>
                <select name="status" id="edit_status" class="form-select rounded-pill" required>
                  @foreach($statusMap as $k => $v)
              ...
              <div class="col-md-6">
                ...
                <select name="payment_status" id="edit_payment_status" class="form-select rounded-pill" required>
                  @foreach($payMap as $k => $v)
                    <option value="{{ $k }}">{{ $v }}</option>
                  @endforeach
                </select>
            ...
            <div class="mt-3">
              ...
              <textarea name="shipping_address" id="edit_address" rows="3" class="form-control rounded-3"></textarea>
            </div>
          </div>
  ...
  <script>
    ...
    function openEditModal(id, status, paymentStatus, address, code) {
