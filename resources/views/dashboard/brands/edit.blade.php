...
@section('page-title', 'Sửa thương hiệu')
...
<div class="container-fluid">
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                <h1 class="mb-2" style="font-size: 2.5rem; font-weight: 700;">
                    <i class="fas fa-edit mr-3"></i>Sửa thông tin thương hiệu
                </h1>
    ...
    <div class="form-container">
        <form action="{{ route('admin.brands.update', $brand->brand_id) }}" method="POST" enctype="multipart/form-data" id="brandForm">
            @csrf
            @method('PUT')
            ...
            <div class="form-card">
                ...
                <div class="form-group">
                    ...
                    @if($brand->logo)
                        <div class="mt-2">
                            <p class="mb-1" style="color: #1e40af; font-weight: 600;">Logo hiện tại:</p>
                            @php
                                $fullPath = storage_path('app/public/' . $brand->logo);
                                if (file_exists($fullPath)) {
                                    echo '<img src="' . asset('storage/' . $brand->logo) . '" alt="Current Logo" class="existing-logo">';
                                } else {
                                    echo '<p style="color: #ef4444;">File không tồn tại tại: ' . $fullPath . '</p>';
                                }
                            @endphp
                        </div>
                    @endif
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon">
                        ...
                        <select name="status" class="form-control form-control-modern @error('status') is-invalid @enderror" required>
                            <option value="1" {{ old('status', $brand->status) == 1 ? 'selected' : '' }}>Đang hoạt động</option>
                            <option value="0" {{ old('status', $brand->status) == 0 ? 'selected' : '' }}>Ngưng hoạt động</option>
                            <option value="2" {{ old('status', $brand->status) == 2 ? 'selected' : '' }}>Hết hàng</option>
                        </select>
                    </div>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
